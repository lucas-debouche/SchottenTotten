...
def entrainement_automatique(plateau, nombre_parties, adversaire=None):
    ...
    :param nombre_parties: Nombre total de parties à jouer pour l'entraînement.
    ...
    for partie in range(1, nombre_parties + 1):
        ...
        plateau.commencer_nouvelle_manche(mode="classic", nbr_manche=1)

        total_reward = 0
        done = False
        while not done:
            joueur = plateau.joueur_actuel

            if plateau.joueurs[joueur].nom == "IA":
                # IA joue son tour
                reward = 0
                current_state_vector = convert_plateau_to_vector(plateau)
                possible_actions = plateau.generate_actions()
                plateau.neural_agent.action_size = len(possible_actions)

                action = plateau.neural_agent.choose_action(
                    current_state_vector, list(range(len(possible_actions)))
                )
                chosen_action = possible_actions[action]

                if isinstance(chosen_action[0], CarteClan):
                    carte, borne = chosen_action
                    reward = plateau.calculate_reward(joueur, carte, borne)
                    plateau.ajouter_carte(borne, joueur, carte, "")
                    plateau.joueurs[joueur].main.remove(carte)

                elif chosen_action[0] == "REVENDIQUER":
                    _, borne = chosen_action
                    reward = plateau.calculate_reward(joueur, numero_borne=borne, revendication=True)
                    plateau.revendiquer_borne(borne, joueur, None, None, None)

                next_state_vector = convert_plateau_to_vector(plateau)
                possible_next_actions = plateau.generate_actions()
                done = not plateau.verifier_fin_manche()

                plateau.neural_agent.train_on_experience(
                    current_state_vector, action, reward, next_state_vector,
                    list(range(len(possible_next_actions))), done
                )

                total_reward += reward
                plateau.neural_agent.decay_exploration_rate()

            else:
                # Adversaire aléatoire par défaut
                actions = plateau.generate_actions()
                action = random.choice(actions)
                if isinstance(action[0], CarteClan):
                    carte, borne = action
                    plateau.ajouter_carte(borne, joueur, carte, "")
                    plateau.joueurs[joueur].main.remove(carte)
                elif action[0] == "REVENDIQUER":
                    _, borne = action
                    plateau.revendiquer_borne(borne, joueur, None, None, None)
        ...
        plateau.neural_agent.log_performance(total_reward)
        performances.append(total_reward)

        # Affichage de progression
        if partie % 100 == 0:
            print(f"Partie {partie}/{nombre_parties} terminée. Total Reward: {total_reward}")

    # Sauvegarder les poids du modèle
    model_path = "neural_agent_model.pth"
    torch.save(plateau.neural_agent.q_network.state_dict(), model_path)
